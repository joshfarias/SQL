...
--
-- Host: localhost
-- Generation Time: Mar 27, 2020 at 04:28 PM
-- Server version: 10.1.38-MariaDB
-- PHP Version: 7.1.27

CREATE DATABASE COMSC210_LAB9;
...
--
-- Stand-in structure for view `vw10most_expensive_products`
...
--
-- Stand-in structure for view `vwcustomer_supplier_by_city`
-- (See below for the actual view)
--
CREATE TABLE `vwcustomer_supplier_by_city` (
`City` varchar(15)
...
--
-- Stand-in structure for view `vwinvoice`
-- (See below for the actual view)
--
CREATE TABLE `vwinvoice` (
`ShipName` varchar(40)
...
--
-- Stand-in structure for view `vwproducts_above_average_price`
-- (See below for the actual view)
--
CREATE TABLE `vwproducts_above_average_price` (
`ProductName` varchar(40)
,`UnitPrice` double
);
...
--
-- Stand-in structure for view `vwquarterly_orders_by_product`
-- (See below for the actual view)
--
CREATE TABLE `vwquarterly_orders_by_product` (
`ProductName` varchar(40)
,`CompanyName` varchar(40)
,`OrderYear` int(4)
,`Qtr 1` varchar(63)
...
--
-- Stand-in structure for view `vwsales_by_category`
-- (See below for the actual view)
--
CREATE TABLE `vwsales_by_category` (
`CategoryID` tinyint(5) unsigned
,`CategoryName` varchar(15)
,`ProductName` varchar(40)
,`ProductSales` double(19,2)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `vwunits_in_stock`
-- (See below for the actual view)
--
CREATE TABLE `vwunits_in_stock` (
`Product Category` varchar(15)
,`Supplier Continent` varchar(12)
,`UnitsInStock` decimal(27,0)
);

-- --------------------------------------------------------

--
-- Structure for view `vw10most_expensive_products`
--
-- DROP TABLE IF EXISTS `vw10most_expensive_products`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw10most_expensive_products`  AS  select distinct `products`.`ProductName` AS `Ten_Most_Expensive_Products`,`products`.`UnitPrice` AS `UnitPrice` from `products` order by `products`.`UnitPrice` desc limit 10 ;

-- --------------------------------------------------------

--
-- Structure for view `vwcustomer_supplier_by_city`
--
DROP TABLE IF EXISTS `vwcustomer_supplier_by_city`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwcustomer_supplier_by_city`  AS  select `customers`.`City` AS `City`,`customers`.`CompanyName` AS `CompanyName`,`customers`.`ContactName` AS `ContactName`,'Customers' AS `Relationship` from `customers` union select `suppliers`.`City` AS `City`,`suppliers`.`CompanyName` AS `CompanyName`,`suppliers`.`ContactName` AS `ContactName`,'Suppliers' AS `Suppliers` from `suppliers` order by `City`,`CompanyName` ;

-- --------------------------------------------------------

--
-- Structure for view `vwinvoice`
--
DROP TABLE IF EXISTS `vwinvoice`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwinvoice`  AS  select distinct `b`.`ShipName` AS `ShipName`,`b`.`ShipAddress` AS `ShipAddress`,`b`.`ShipCity` AS `ShipCity`,`b`.`ShipRegion` AS `ShipRegion`,`b`.`ShipPostalCode` AS `ShipPostalCode`,`b`.`ShipCountry` AS `ShipCountry`,`b`.`CustomerID` AS `CustomerID`,`c`.`CompanyName` AS `CompanyName`,`c`.`Address` AS `Address`,`c`.`City` AS `City`,`c`.`Region` AS `Region`,`c`.`PostalCode` AS `PostalCode`,`c`.`Country` AS `Country`,concat(`d`.`FirstName`,' ',`d`.`LastName`) AS `Salesperson`,`b`.`OrderID` AS `OrderID`,`b`.`OrderDate` AS `OrderDate`,`b`.`RequiredDate` AS `RequiredDate`,`b`.`ShippedDate` AS `ShippedDate`,`a`.`CompanyName` AS `ShipperName`,`e`.`ProductID` AS `ProductID`,`f`.`ProductName` AS `ProductName`,`e`.`UnitPrice` AS `UnitPrice`,`e`.`Quantity` AS `Quantity`,`e`.`Discount` AS `Discount`,((`e`.`UnitPrice` * `e`.`Quantity`) * (1 - `e`.`Discount`)) AS `ExtendedPrice`,`b`.`Freight` AS `Freight` from (((((`shippers` `a` join `orders` `b` on((`a`.`ShipperID` = `b`.`ShipVia`))) join `customers` `c` on((`c`.`CustomerID` = `b`.`CustomerID`))) join `employees` `d` on((`d`.`EmployeeID` = `b`.`EmployeeID`))) join `order_details` `e` on((`b`.`OrderID` = `e`.`OrderID`))) join `products` `f` on((`f`.`ProductID` = `e`.`ProductID`))) order by `b`.`ShipName` ;

-- --------------------------------------------------------

--
-- Structure for view `vwproducts_above_average_price`
--
DROP TABLE IF EXISTS `vwproducts_above_average_price`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwproducts_above_average_price`  AS  select distinct `products`.`ProductName` AS `ProductName`,`products`.`UnitPrice` AS `UnitPrice` from `products` where (`products`.`UnitPrice` > (select avg(`products`.`UnitPrice`) from `products`)) order by `products`.`UnitPrice` ;

-- --------------------------------------------------------

--
-- Structure for view `vwquarterly_orders_by_product`
--
DROP TABLE IF EXISTS `vwquarterly_orders_by_product`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwquarterly_orders_by_product`  AS  select `a`.`ProductName` AS `ProductName`,`d`.`CompanyName` AS `CompanyName`,year(`c`.`OrderDate`) AS `OrderYear`,format(sum((case quarter(`c`.`OrderDate`) when '1' then ((`b`.`UnitPrice` * `b`.`Quantity`) * (1 - `b`.`Discount`)) else 0 end)),0) AS `Qtr 1`,format(sum((case quarter(`c`.`OrderDate`) when '2' then ((`b`.`UnitPrice` * `b`.`Quantity`) * (1 - `b`.`Discount`)) else 0 end)),0) AS `Qtr 2`,format(sum((case quarter(`c`.`OrderDate`) when '3' then ((`b`.`UnitPrice` * `b`.`Quantity`) * (1 - `b`.`Discount`)) else 0 end)),0) AS `Qtr 3`,format(sum((case quarter(`c`.`OrderDate`) when '4' then ((`b`.`UnitPrice` * `b`.`Quantity`) * (1 - `b`.`Discount`)) else 0 end)),0) AS `Qtr 4` from (((`products` `a` join `order_details` `b` on((`a`.`ProductID` = `b`.`ProductID`))) join `orders` `c` on((`c`.`OrderID` = `b`.`OrderID`))) join `customers` `d` on((`d`.`CustomerID` = `c`.`CustomerID`))) where (`c`.`OrderDate` between cast('1997-01-01' as date) and cast('1997-12-31' as date)) group by `a`.`ProductName`,`d`.`CompanyName`,year(`c`.`OrderDate`) order by `a`.`ProductName`,`d`.`CompanyName` ;

-- --------------------------------------------------------

--
-- Structure for view `vwsales_by_category`
--
DROP TABLE IF EXISTS `vwsales_by_category`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwsales_by_category`  AS  select distinct `a`.`CategoryID` AS `CategoryID`,`a`.`CategoryName` AS `CategoryName`,`b`.`ProductName` AS `ProductName`,sum(round(((`y`.`UnitPrice` * `y`.`Quantity`) * (1 - `y`.`Discount`)),2)) AS `ProductSales` from (((`order_details` `y` join `orders` `d` on((`d`.`OrderID` = `y`.`OrderID`))) join `products` `b` on((`b`.`ProductID` = `y`.`ProductID`))) join `categories` `a` on((`a`.`CategoryID` = `b`.`CategoryID`))) where (`d`.`OrderDate` between cast('1997/1/1' as date) and cast('1997/12/31' as date)) group by `a`.`CategoryID`,`a`.`CategoryName`,`b`.`ProductName` order by `a`.`CategoryName`,`b`.`ProductName`,sum(round(((`y`.`UnitPrice` * `y`.`Quantity`) * (1 - `y`.`Discount`)),2)) ;

-- --------------------------------------------------------

--
-- Structure for view `vwunits_in_stock`
--
DROP TABLE IF EXISTS `vwunits_in_stock`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vwunits_in_stock`  AS  select `c`.`CategoryName` AS `Product Category`,(case when (`s`.`Country` in ('UK','Spain','Sweden','Germany','Norway','Denmark','Netherlands','Finland','Italy','France')) then 'Europe' when (`s`.`Country` in ('USA','Canada','Brazil')) then 'America' else 'Asia-Pacific' end) AS `Supplier Continent`,sum(`p`.`UnitsInStock`) AS `UnitsInStock` from ((`suppliers` `s` join `products` `p` on((`p`.`SupplierID` = `s`.`SupplierID`))) join `categories` `c` on((`c`.`CategoryID` = `p`.`CategoryID`))) group by `c`.`CategoryName`,(case when (`s`.`Country` in ('UK','Spain','Sweden','Germany','Norway','Denmark','Netherlands','Finland','Italy','France')) then 'Europe' when (`s`.`Country` in ('USA','Canada','Brazil')) then 'America' else 'Asia-Pacific' end) ;
